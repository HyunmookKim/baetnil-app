...
//   국립해양조사원 조위는 약최저저조위(기본수준면)에서 잰 높이이므로
//
//        저조위 기준 = 잰 수심 − 그 시각 조위
//
//   핀을 찍었으니 앱이 할 수 있는 일은 이것이다 —
//   그 자리에서 가장 가까운 조위관측소를 알아서 잡고, 잰 시각 조위를 빼 준다.
...
// ── 4. 핀 자리의 그 시각 조위
{
  ...
  try{
    ...
    const fn = new Function('tideCache', 'tidePackSpots', 'SPOT_PACKS', 'hcTidePts',
      // ★ 5.10 — 관 물때표 거리 한도(tideOfficialNear, HC_NEAR_KM)도 같이 넣는다
  ...
  if(out){
    ...
    // ★ 5.10 — 150km 넘게 떨어진 관측소 물때를 그 자리 것처럼 쓰지 않는다 (블라디보스토크에서 여수가 떴던 것)
    try{
      const far = out.먼곳;
