...
(async()=>{
  ...
  await pg.screenshot({path:'cmp_legal.png'});
  const info = await pg.evaluate(()=>{
    const h=document.querySelector('#mrPanel .mrhead');
    const r=h?h.getBoundingClientRect():null;
    const n=document.querySelector('#mrPanel .mrnone');
    return { head: h? getComputedStyle(h).display:'(없음)', 높이:r?Math.round(r.height):0,
             mrnoneColor: n?getComputedStyle(n).color:'(없음)',
             panelBg: getComputedStyle(document.getElementById('mrPanel')).backgroundColor,
             headTitle: (document.getElementById('headTitle')||{}).textContent };
  });
