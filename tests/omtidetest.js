...
T('해상 자료를 부르는 자리는 둘뿐이다 (배 자리 · 둘레 바다)',
  (src.match(/marine-api\.open-meteo\.com/g) || []).length === 2);
...
T('★★★ 왜 안 하는지 말해 준다', /참고용 물때에는 해도 기준면이 없습니다/.test(src));
...
// ── 화면이 사실대로 말한다
T('★ 참고용일 때 경고줄이 달라진다', /TA && TA\.est\s*\n?\s*\? t\('이 위치는 가까이에 조위 관측소가 없어/.test(src));
...
['관측소 없음',
 ...
 '참고용 물때에는 해도 기준면이 없습니다. 만조·간조를 직접 입력하시면 계산해 드립니다.',
